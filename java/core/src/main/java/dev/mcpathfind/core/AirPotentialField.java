...
public final class AirPotentialField {
    ...
    public static AirPotentialField build(World world, int startX, int startY, int startZ) {
        ...
        boolean[] chunkHasAir = new boolean[numChunks];
        for (int cx = 0; cx < chunksX; cx++) {
            for (int cy = 0; cy < chunksY; cy++) {
                for (int cz = 0; cz < chunksZ; cz++) {
                    chunkHasAir[(cx * chunksY + cy) * chunksZ + cz] =
                            chunkContainsAir(world, cx, cy, cz);
                }
            }
        }
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] n : neighbors6) {
                ...
                if (!chunkHasAir[nIdx] || dist[nIdx] != UNREACHED) {
                    continue;
                }
                dist[nIdx] = curDist + 1;
