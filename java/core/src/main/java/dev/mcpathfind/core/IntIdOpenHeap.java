...
/**
 * Binary min-heap over dense int state IDs (see StateIdMap) -- the
 * decrease-key successor to LongDoubleOpenHeap.
 *
 * The first decrease-key attempt (see git history / conversation) tracked
 * each packed-long state's heap position in a Long2IntOpenHashMap. That
 * correctly eliminated duplicate open-entries for the same state (which had
 * been causing a 2.59x push-to-expansion bloat and ~31% stale pops on real
 * search workloads), but the fix was NET SLOWER: every swap() during
 * sifting -- which happens far more often than pushes -- now paid two
 * hashmap put()s to keep the index in sync, and that cost exceeded the
 * savings from fewer duplicate entries. Profiling confirmed it: total wall
 * time went UP (~4.1s -> ~5.0-5.3s on k2_r_0_0's success-case rung).
 *
 * This version fixes the actual mistake: positions are tracked in a flat
 * int[] indexed by the state's DENSE id (0, 1, 2, ... from StateIdMap),
 * not a hash map keyed by the raw packed long. Array reads/writes are
 * effectively free compared to hashing -- so decrease-key's bookkeeping on
 * every swap now costs the same as any other array write, instead of a
 * hashmap put. This is the whole point of routing everything through
 * StateIdMap first.
 *
...
public final class IntIdOpenHeap {
     ...
     * call this when the new priority is a genuine improvement
     * (WeightedAStar's existing "tentativeG < gScore[id]" guard already
     * establishes this), but this also double-checks, matching the old
     * heap's behavior.
     */
     ...
     * insertion order. The y tiebreak reflects a real domain heuristic
     * (see WeightedAStar's class javadoc / the session that added this):
     * for similar terrain, higher ground is almost always at least as
     * good as lower ground, since falling back down is cheap (a FALL
     * action) but climbing back up never is -- so among otherwise-tied
     * candidates, explore the higher one first.
     */
